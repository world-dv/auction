...
public class ProductController {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Product registered successfully."), @ApiResponse(responseCode = "404", description = "Auction not found.")})
    @PostMapping(consumes = {MediaType.MULTIPART_FORM_DATA_VALUE})
    public ResponseEntity<ApiResult<ProductResponse>> registerProduct(@RequestParam Long userId, @RequestParam Long auctionId, @RequestPart("productRequest") ProductRequest.Register productRequest, @RequestPart("images") List<MultipartFile> images) {
    ...
    @ApiResponses(value = {@ApiResponse(responseCode = "200", description = "Product(s) retrieved successfully."), @ApiResponse(responseCode = "404", description = "Product(s) not found for the given user ID or auction ID.")})
    @GetMapping
    public ResponseEntity<ApiResult<?>> getProducts(@RequestParam Long auctionId) {
